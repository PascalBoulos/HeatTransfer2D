...
equations(2).nDim=2;
equations(2).formulationFaible=@formeFaibleDiffusion;
equations(2).parametres=1;
equations(2).solutionInitiale=0.5;

% Equation 2

equations(1).domaines=2;
equations(1).domainNames='murGauche';
equations(1).variables='T';
equations(1).nDegs=3;
equations(1).nDegPoidsInterpolation=6;
equations(1).nDim=1;
equations(1).formulationFaible=@dirichlet;
equations(1).dirichlet=0;
